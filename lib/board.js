(function(){
  ...
  board.prototype.render = function ($el) {
    ...
    this.grid.forEach(function (cell) {
      ...
      if (that._include(that.snake1._segments, cell)) {
        $cell.css("background", that.snake1._color);
      } else if(that.players > 1 && that._include(that.snake2._segments, cell)) {
        $cell.css("background", that.snake2._color);
  ...
  board.prototype._getSnakeHeads = function () {
    var heads = [];
    if (this.players > 1) {
      var head2 = this.snake2._segments[this.snake2._segments.length - 1];
      heads.push(head2);
    }
    var head1 = this.snake1._segments[this.snake1._segments.length - 1];
    heads.push(head1);
    return heads;
  };
  ...
  board.prototype.setApple = function () {
    if (this.players > 1) {
      ...
      } while (this._include(this.snake2._segments, this.apple) || this._include(this.snake1._segments, this.apple) );
    } else {
      do {
        this.apple[0] = Math.floor(Math.random() * 20);
        this.apple[1] = Math.floor(Math.random() * 20);
      } while (this._include(this.snake1._segments, this.apple));
    }
  ...
  board.prototype.checkSnakes = function () {
    ...
    var snakeSansHead1 = this.snake1._segments.slice(0, this.snake1._segments.length - 2);

    if (this.players > 1) {
      var head2 = this.snake2.headDup();
      var snakeSansHead2 = this.snake2._segments.slice(0, this.snake2._segments.length - 2);
      if (this._include(snakeSansHead1, head1) || this._include(snakeSansHead2, head2)  ) {//snakes eat themselves.

          this.keepRendering = false;
          return false;
      } else if (this._include(this.snake1._segments, head2)) {//snake2 eats snake1
        this.snake2.score = 0;
        this.keepRendering = false;
        return false;

      } else if (this._include(this.snake2._segments, head1)){//snake1 eats snake2
        this.snake1.score = 0;
