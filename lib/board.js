(function(){
  ...
  board.prototype.render = function ($el) {
    ...
    this.grid.forEach(function (cell) {
      ...
      if (that._include(that.snake1.position(), cell)) {
        $cell.css("background-color", that.snake1.getColor());
      } else if(that.players > 1 && that._include(that.snake2.position(), cell)) {
        $cell.css("background-color", that.snake2.getColor());
  ...
  board.prototype._getSnakeHeads = function () {
    var heads = [];
    if (this.players > 1) {
      var head2 = this.snake2.position()[this.snake2.position().length - 1];
      heads.push(head2);
    }
    var head1 = this.snake1.position()[this.snake1.position().length - 1];
    heads.push(head1);
    return heads;
  };
  ...
  board.prototype.checkSnakes = function () {
    ...
    var snakeSansHead1 = this.snake1.position().slice(0, this.snake1.position().length - 2);

    if (this.players > 1) {
      var head2 = this.snake2.headDup();
      var snakeSansHead2 = this.snake2.position().slice(0, this.snake2.position().length - 2);
